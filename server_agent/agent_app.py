...
def load_schema_instructions(models_file_path='database/models.py'):
    ...
    with open(models_file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    pattern = re.compile('class\\s+(\\w+)\\(Base\\)')
    ...
    instructions.append("\nIf user says 'purchase orders', interpret as 'purchase_order'. If user says 'parent of detail_item', interpret as 'purchase_order'.\nFor best practices or coding strategy, respond with action='other'.")
    return '\n'.join(instructions)
...
class AgentApp:
    ...
    def _interpret_intent(self, user_text: str) -> dict:
        ...
        """
        system_prompt = f'You are an AI that handles DB requests or best practices.\nReturn JSON with:\n  action in [create_table, update_table, delete_table, insert_data, update_data, query_data, other]\n  table_name\n  columns_or_updates\n  where\n  filters\n\nBelow is the current schema context:\n{self.schema_docs}\n'
        messages = [{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': user_text}]
        try:
            response = self.client.chat.completions.create(model='gpt-3.5-turbo', messages=messages, max_tokens=512, temperature=0)
            content = response.choices[0].message.content.strip()
            parsed = json.loads(content)
            return parsed
        except Exception as e:
            self.logger.error(f'[_interpret_intent] - Error parsing user intent: {e}', exc_info=True)
    ...
    def _handle_intent(self, intent: dict, user_text: str):
        ...
        if action == 'create_table':
            ...
            self.logger.info('[_handle_intent] - No direct DB action recognized; treating as general question.')
