    ...
    if new_state in {"REVIEWED", "PO MISMATCH"} and due_date:
        try:
            if isinstance(due_date, str):
                # Convert from string like "2025-01-16"
                ...
                # If it's none of the above, handle or log
                due_dt = None
...
def _ensure_bill_line_item(detail_item: dict, parent_id: int):
    ...
    # --- NEW/UPDATED CODE: Begin: Budget/Tax Code Lookup ---
    numeric_tax_code = None

    # 1) Look up the Project to see which budget_map_id + tax_ledger to use
    try:
        ...
        if project_records:
            ...
            # Example: we’re pulling out "OPH_2024" from the project record
            budget_map_id = project.get("budget_map_id")  # e.g. "1" or similar
            logger.info(
                f"[some_trigger_logic] Found project for project_number={detail_item['project_number']} -> "
                f"budget_map_id={budget_map_id}."
            )
        else:
            logger.warning(
                f"[some_trigger_logic] Could not find project with project_number={detail_item['project_number']}."
            )
            budget_map_id = None
    except Exception as e:
        logger.warning(
            f"[some_trigger_logic] Failed to fetch project for project_number={detail_item['project_number']}: {e}"
        )
        budget_map_id = None

    # 2) If we have a budget_map_id, find the BudgetMap
    if budget_map_id:
        try:
            ...
            if budget_maps:
                ...
                logger.info(
                    f"[some_trigger_logic] Found BudgetMap(id={budget_map_id}) "
                    f"for map_name='{budget_map_name}'."
                )
            else:
                logger.warning(
                    f"[some_trigger_logic] No BudgetMap found for ID= {budget_map_id}'."
                )
        except Exception as e:
            logger.warning(
                f"[some_trigger_logic] Failed to fetch BudgetMap with ID='{budget_map_id}': {e}"
            )

    # 3) Find the matching AccountCode record for detail_item.account_code & budget_map_id
    account_code_id = None
    if budget_map_id:
        try:
            logger.info(
                f"[some_trigger_logic] Searching AccountCodes for code='{detail_item['account_code']}' "
                f"with budget_map_id={budget_map_id}."
            )
            ...
            if account_codes:
                ...
                logger.info(
                    f"[some_trigger_logic] Found AccountCode(id={account_code_id}). "
                    f"tax_id={tax_id}. detail_item.account_code={detail_item['account_code']}"
                )

                # 4) Retrieve the actual TaxAccount record to get the numeric tax_code (e.g. 5300)
                if tax_id:
                    try:
                        tax_record = db_ops.search_tax_accounts(["id"], [tax_id])
                        if tax_record and not isinstance(tax_record, list):
                            numeric_tax_code = tax_record.get("tax_code")  # "5300"
                            logger.info(
                                f"[some_trigger_logic] TaxAccount found for id={tax_id}, "
                                f"numeric_tax_code='{numeric_tax_code}'."
                            )
                        else:
                            logger.info(
                                f"[some_trigger_logic] No single TaxAccount record found for id={tax_id} "
                                f"(or returned multiple)."
                            )
                    except Exception as e:
                        logger.warning(
                            f"[some_trigger_logic] Failed to fetch TaxAccount(id={tax_id}): {e}"
                        )
                else:
                    logger.info(
                        f"[some_trigger_logic] tax_id was None, so no TaxAccount is set for this AccountCode."
                    )
            else:
                logger.warning(
                    f"[some_trigger_logic] No AccountCode found for code='{detail_item['account_code']}' "
                    f"and budget_map_id={budget_map_id}."
                )
        except Exception as e:
            logger.warning(
                f"[some_trigger_logic] Failed searching AccountCodes with code='{detail_item['account_code']}', "
                f"budget_map_id={budget_map_id}: {e}"
            )

    # Now you can inspect numeric_tax_code vs. detail_item["account_code"]
    # to decide which one to store as BillLineItem.account_code, etc.
    logger.info(
        f"[some_trigger_logic] Final numeric_tax_code={numeric_tax_code} for detail_item.account_code="
        f"{detail_item['account_code']}."
    )

    # --- NEW/UPDATED CODE: End: Budget/Tax Code Lookup ---

    # If no numeric_tax_code is found, you can decide to fallback to something:
    # numeric_tax_code = numeric_tax_code or 9999

    project_number = detail_item["project_number"]
    po_number = detail_item["po_number"]
    detail_number = detail_item["detail_number"]

    # For BillLineItem.account_code, use the numeric_tax_code if found;
    # otherwise, you might store the detail_item's own account_code or fallback.
    # For example:
    ...
    if found:
        ...
        needs_update = (
            ...
            or found.get("code") != final_account_code   # <--- compare numeric code
            or found.get("project_number") != project_number
            or found.get("po_number") != po_number
            or found.get("detail_number") != detail_number
        )
        ...
        if needs_update:
            try:
                updated_line = db_ops.update_bill_line_item(
                    ...
                    account_code=final_account_code,  # store numeric tax code here
                    project_number=project_number,
                    po_number=po_number,
                    detail_number=detail_number,
                )
    ...
    # If no BillLineItem is found, create a new one
    try:
