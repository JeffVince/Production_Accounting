...
def _update_xero_bill_dates(xero_bill: Dict[str, Any]) -> None:
    ...
    logger.debug(f'[_update_xero_bill_dates] Checking earliest/lates dates for XeroBill(id={bill_id}), project_number={project_number}, po_number={po_number}, detail_number={detail_number}')
    detail_items = db_ops.search_detail_items(['project_number', 'po_number', 'detail_number'], [project_number, po_number, detail_number])
    if not detail_items:
    ...
    logger.debug(f'[_update_xero_bill_dates] Found {len(detail_items)} DetailItems. TransactionDates={transaction_dates}, DueDates={due_dates}')
    ...
    if updated_fields:
        ...
        if updated_record:
            logger.info(f'[_update_xero_bill_dates] Updated XeroBill(id={bill_id}) with {updated_fields}')
        else:
            logger.warning(f'[_update_xero_bill_dates] Failed to update XeroBill(id={bill_id}).')
...
def handle_xero_bill_create(bill_id: int) -> None:
    ...
    if not xero_id:
        logger.info(f"[BILL CREATE] No xero_id. Searching Xero by reference='{reference}'.")
        ...
        if not matched:
            ...
            created_invoices = xero_api._retry_on_unauthorized(xero_api.xero.invoices.put, [{'Type': 'ACCPAY', 'Reference': reference}])
            ...
            try:
                ...
                logger.info(f'[BILL CREATE] Created new invoice in Xero. Bill(id={bill_id}) => xero_id={new_xero_id}.')
        ...
        if not xero_link:
            ...
            if invoice_obj:
                ...
                logger.info(f"[BILL CREATE] Stored xero_link='{link}' for Bill(id={bill_id}).")
            else:
                logger.warning(f'[BILL CREATE] Could not fetch Xero invoice with ID={xero_id}, no link stored.')
    ...
    if not xero_id:
        ...
        if not xero_link:
            ...
            if invoice_data:
                ...
                logger.warning(f'[BILL UPDATE] Could not fetch Xero invoice with xero_id={xero_id} to build link.')
    _update_xero_bill_dates(xero_bill)
