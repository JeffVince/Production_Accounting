...
def get_map_data():
    """
    We interpret ?sort_by=<col>_asc or <col>_desc
    e.g. sort_by=code_asc => sort by code ascending
         sort_by=description_desc => etc
    """
    try:
        ...
        raw_sort = request.args.get("sort_by","code_natural_asc")  # e.g. code_natural_asc
        # parse the raw_sort
        sort_parts=raw_sort.split("_")
        if len(sort_parts)>=2:
            col=sort_parts[0]
...
def create_map_code():
    try:
        ...
        db_view_util.create_map_code(new_map_code, copy_from, user_id=1)
...
def add_ledger():
    try:
        ...
        if not ledger_name:
            return jsonify({"status":"error","message":"ledger_name is required"}),400

        ledger_id, actual_name = db_view_util.add_ledger_custom(
            current_map=map_name,
            ledger_name=ledger_name,
            user_id=1
        )
        return jsonify({"status":"success","ledger_id":ledger_id,"actual_ledger_name":actual_name}),200
    except SQLAlchemyError as e:
...
def create_tax_code():
    try:
        ...
        if not ledger_id or not tax_code:
            return jsonify({"status":"error","message":"ledger_id and tax_code required"}),400
        new_id=db_view_util.create_tax_record(tax_code, description, int(ledger_id))
...
def update_tax():
    try:
        ...
        if not tax_id or not tax_code:
            return jsonify({"status":"error","message":"tax_id and tax_code required"}),400
...
def assign_tax_bulk():
    try:
        ...
        if not map_name or not account_ids or not tax_id:
            return jsonify({"status":"error","message":"map_name, account_ids, tax_id are required"}),400
