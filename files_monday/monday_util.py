...
class MondayUtil(metaclass=SingletonMeta):
    ...
    MAIN_ITEM_COLUMN_ID_TO_DB_FIELD = {'id': 'pulse_id', PO_PROJECT_ID_COLUMN: 'project_id', PO_NUMBER_COLUMN: 'po_number', PO_TAX_COLUMN_ID: 'tax_form_link', PO_DESCRIPTION_COLUMN_ID: 'description', PO_CONTACT_CONNECTION_COLUMN_ID: 'contact_id', PO_FOLDER_LINK_COLUMN_ID: 'folder_link', PO_PRODUCER_COLUMN_ID: 'producer'}
    SUB_ITEM_COLUMN_ID_TO_DB_FIELD = {SUBITEM_STATUS_COLUMN_ID: 'state', SUBITEM_ID_COLUMN_ID: 'detail_number', SUBITEM_DESCRIPTION_COLUMN_ID: 'description', SUBITEM_QUANTITY_COLUMN_ID: 'quantity', SUBITEM_RATE_COLUMN_ID: 'rate', SUBITEM_DATE_COLUMN_ID: 'transaction_date', SUBITEM_ACCOUNT_NUMBER_COLUMN_ID: 'account_number', SUBITEM_LINK_COLUMN_ID: 'file_link'}
    CONTACT_COLUMN_ID_TO_DB_FIELD = {CONTACT_PHONE: 'phone', CONTACT_EMAIL: 'email', CONTACT_ADDRESS_LINE_1: 'address_line_1', CONTACT_ADDRESS_LINE_2: 'address_line_2', CONTACT_ADDRESS_CITY: 'city', CONTACT_ADDRESS_ZIP: 'zip', CONTACT_REGION: 'region', CONTACT_ADDRESS_COUNTRY: 'country', CONTACT_TAX_TYPE: 'tax_type', CONTACT_TAX_NUMBER: 'tax_ID', CONTACT_PAYMENT_DETAILS: 'payment_details'}
    COLUMN_TYPE_HANDLERS = {'dropdown': 'handle_dropdown_column', 'default': 'handle_default_column', 'date': 'handle_date_column', 'color': 'handle_status_column', 'link': 'handle_link_column', 'text': 'handle_default_column'}
            ...
            try:
                ...
                for column in columns:
                    if column['type'] == 'subtasks':
                        self.logger.debug(f"[get_subitems_column_id] - Found subitems column ID: {column['id']}")
                        return column['id']
            except Exception as e:
                self.logger.error(f'[get_subitems_column_id] - Failed to retrieve columns: {data}')
    ...
    def _handle_link_column(self, event):
        ...
        try:
            ...
            self.logger.warning('[_handle_link_column] - Setting link to None because of unexpected Monday Link Value.')
            return None
    ...
    def _handle_dropdown_column(self, event):
        ...
        try:
            ...
            self.logger.warning('[_handle_dropdown_column] - Setting Account ID to None because of unexpected Monday Account Value.')
            return None
    ...
    def create_item(self, group_id, item_name, column_values):
        ...
        variables = {'board_id': self.PO_BOARD_ID, 'group_id': group_id, 'item_name': item_name, 'column_values': serialized_column_values}
        self.logger.info(f'[create_item] - Creating item with variables: {variables}')
        response = requests.post(self.MONDAY_API_URL, headers=self.headers, json={'query': query, 'variables': variables})
        data = response.json()
        if response.status_code == 200:
            if 'data' in data and 'create_item' in data['data']:
                item_id = data['data']['create_item']['id']
                self.logger.info(f"[create_item] - Created new item '{item_name}' with ID {item_id}")
                return item_id
            elif 'errors' in data:
                self.logger.error(f"[create_item] - Error creating item in Monday.com: {data['errors']}")
                return None
            else:
                self.logger.error(f'[create_item] - Unexpected response structure: {data}')
                return None
        else:
            self.logger.error(f'[create_item] - HTTP Error {response.status_code}: {response.text}')
            return None
    ...
    def update_item_columns(self, item_id, column_values, board='po'):
        ...
        self.logger.info(f'[update_item_columns] - Updating item {item_id} with columns: {column_values}')
        response = requests.post(self.MONDAY_API_URL, headers=self.headers, json={'query': query})
        data = response.json()
        if response.status_code == 200:
            if 'data' in data:
                self.logger.info(f'[update_item_columns] - Successfully updated item {item_id} in Monday.com.')
                return True
            elif 'errors' in data:
                self.logger.error(f"[update_item_columns] - Error updating item in Monday.com: {data['errors']}")
                return False
            else:
                self.logger.error(f'[update_item_columns] - Unexpected response structure: {data}')
                return False
        else:
            self.logger.error(f'[update_item_columns] - HTTP Error {response.status_code}: {response.text}')
            return False

    def po_column_values_formatter(self, project_id=None, po_number=None, tax_id=None, description=None, contact_pulse_id=None, folder_link=None, status=None, producer_id=None, name=None):
        column_values = {}
        if project_id:
    ...
    def subitem_column_values_formatter(self, project_id=None, po_number=None, detail_number=None, line_number=None, notes=None, status=None, description=None, quantity=None, rate=None, date=None, due_date=None, account_number=None, link=None, OT=None, fringes=None, xero_link=None):
        column_values = {}
        if notes:
        ...
        if quantity is not None:
            try:
                ...
                self.logger.error(f"[subitem_column_values_formatter] - Invalid quantity '{quantity}': {e}")
        ...
        if rate is not None:
            try:
                ...
                self.logger.error(f"[subitem_column_values_formatter] - Invalid rate '{rate}': {e}")
        ...
        if OT is not None:
            try:
                ...
                self.logger.error(f"[subitem_column_values_formatter] - Invalid OT '{OT}': {e}")
        ...
        if fringes is not None:
            try:
                ...
                self.logger.error(f"[subitem_column_values_formatter] - Invalid fringes '{fringes}': {e}")
        ...
        if date:
            try:
                ...
                self.logger.error(f"[subitem_column_values_formatter] - Error parsing date '{date}': {e}")
        if due_date:
            try:
                ...
                self.logger.error(f"[subitem_column_values_formatter] - Error parsing due_date '{due_date}': {e}")
                raise
        if account_number:
            try:
                ...
                self.logger.error(f"[subitem_column_values_formatter] - Invalid account number '{account_number}': {e}")
    ...
    def create_subitem(self, parent_item_id, subitem_name, column_values):
        ...
        self.logger.info(f"[create_subitem] - Creating subitem under parent {parent_item_id} with name '{subitem_name}'.")
        response = requests.post(self.MONDAY_API_URL, headers=self.headers, json={'query': query})
        data = response.json()
        if response.status_code == 200:
            if 'data' in data and 'create_subitem' in data['data']:
                subitem_id = data['data']['create_subitem']['id']
                self.logger.info(f'[create_subitem] - Created subitem with ID {subitem_id}')
                return subitem_id
            elif 'errors' in data:
                self.logger.error(f"[create_subitem] - Error creating subitem in Monday.com: {data['errors']}")
                return None
            else:
                self.logger.error(f'[create_subitem] - Unexpected response structure: {data}')
                return None
        else:
            self.logger.error(f'[create_subitem] - HTTP Error {response.status_code}: {response.text}')
            return None
    ...
    def update_subitem_columns(self, subitem_id, column_values):
        ...
        column_values_json = json.dumps(column_values).replace('"', '\\"')
        mutation = f'\n        mutation {{\n            change_multiple_column_values(\n                board_id: {self.SUBITEM_BOARD_ID},\n                item_id: {subitem_id},\n                column_values: "{column_values_json}"\n            ) {{\n                id\n            }}\n        }}\n        '
        self.logger.info(f'[update_subitem_columns] - Updating subitem {subitem_id} with columns: {column_values}')
        response = requests.post(self.MONDAY_API_URL, headers=self.headers, json={'query': mutation})
        data = response.json()
        if response.status_code == 200:
            if 'data' in data:
                self.logger.info(f'[update_subitem_columns] - Successfully updated subitem {subitem_id} in Monday.com.')
                return True
            elif 'errors' in data:
                self.logger.error(f"[update_subitem_columns] - Error updating subitem in Monday.com: {data['errors']}")
                return False
            else:
                self.logger.error(f'[update_subitem_columns] - Unexpected response structure: {data}')
                return False
        else:
            self.logger.error(f'[update_subitem_columns] - HTTP Error {response.status_code}: {response.text}')
            return False
    ...
    def link_contact_to_po_item(self, po_item_id, contact_item_id):
        ...
        column_value = {'item_ids': [contact_item_id]}
        column_value_json = json.dumps(column_value).replace('"', '\\"')
        mutation = f'\n        mutation {{\n            change_column_value(\n                board_id: {self.PO_BOARD_ID},\n                item_id: {po_item_id},\n                column_id: "{connect_boards_column_id}",\n                value: "{column_value_json}"\n            ) {{\n                id\n            }}\n        }}\n        '
        self.logger.info(f'[link_contact_to_po_item] - Linking contact {contact_item_id} to PO item {po_item_id}.')
        response = requests.post(self.MONDAY_API_URL, headers=self.headers, json={'query': mutation})
        data = response.json()
        if response.status_code == 200:
            if 'data' in data and 'change_column_value' in data['data']:
                self.logger.info(f'[link_contact_to_po_item] - Successfully linked contact item {contact_item_id} to PO item {po_item_id}.')
                return True
            elif 'errors' in data:
                self.logger.error(f"[link_contact_to_po_item] - Error linking contact to PO item in Monday.com: {data['errors']}")
                return False
            else:
                self.logger.error(f'[link_contact_to_po_item] - Unexpected response structure: {data}')
                return False
        else:
            self.logger.error(f'[link_contact_to_po_item] - HTTP Error {response.status_code}: {response.text}')
            return False
    ...
    def validate_monday_request(self, request_headers):
        ...
        if not token:
            self.logger.warning("[validate_monday_request] - Missing 'Authorization' header.")
            return False
        try:
            received_token = token.split()[1]
        except IndexError:
            self.logger.warning("[validate_monday_request] - Invalid 'Authorization' header format.")
            return False
    ...
    def is_main_item_different(self, db_item, monday_item):
        ...
        if 'connect_boards1' in col_vals and col_vals['connect_boards1']:
            ...
            linked_pulse_id = None
        field_map = [{'field': 'project_number', 'db_value': db_item.get('project_number'), 'monday_value': col_vals.get('project_id')['text']}, {'field': 'contact_name', 'db_value': db_item.get('contact_name'), 'monday_value': monday_item.get('name')}, {'field': 'PO', 'db_value': str(db_item.get('po_number')), 'monday_value': col_vals.get('numeric__1')['text']}, {'field': 'description', 'db_value': db_item.get('description'), 'monday_value': col_vals.get('text6')['text']}, {'field': 'Connected Contact', 'db_value': db_item.get('contact_pulse_id'), 'monday_value': linked_pulse_id}]
        for f in field_map:
            ...
            if db_str != mon_str:
                differences.append({'field': f['field'], 'db_value': db_str, 'monday_value': mon_str})
        return differences
    ...
    def is_sub_item_different(self, db_sub_item, monday_sub_item):
        ...
        field_map = [{'field': 'quantity', 'db_value': safe_str(db_sub_item.get('quantity')), 'monday_value': safe_str(col_vals.get(self.SUBITEM_QUANTITY_COLUMN_ID)['text'])}, {'field': 'file_link', 'db_value': safe_str(db_sub_item.get('file_link')), 'monday_value': safe_str(json.loads(col_vals.get(self.SUBITEM_LINK_COLUMN_ID)['value'])['url']) if col_vals.get(self.SUBITEM_LINK_COLUMN_ID)['value'] else ''}, {'field': 'detail_number', 'db_value': safe_str(db_sub_item.get('detail_number')), 'monday_value': safe_str(col_vals.get(self.SUBITEM_ID_COLUMN_ID)['text'])}, {'field': 'line_number', 'db_value': safe_str(db_sub_item.get('line_number')), 'monday_value': safe_str(col_vals.get(self.SUBITEM_LINE_NUMBER_COLUMN_ID)['text'])}, {'field': 'rate', 'db_value': safe_str(db_sub_item.get('rate')), 'monday_value': safe_str(col_vals.get(self.SUBITEM_RATE_COLUMN_ID)['text'])}, {'field': 'account_code', 'db_value': safe_str(db_sub_item.get('account_code')), 'monday_value': safe_str(col_vals.get(self.SUBITEM_ACCOUNT_NUMBER_COLUMN_ID)['text'])}, {'field': 'ot', 'db_value': safe_str(db_sub_item.get('ot')), 'monday_value': safe_str(col_vals.get(self.SUBITEM_OT_COLUMN_ID)['text'])}, {'field': 'state', 'db_value': safe_str(db_sub_item.get('state')), 'monday_value': safe_str(col_vals.get(self.SUBITEM_STATUS_COLUMN_ID)['text'])}, {'field': 'fringes', 'db_value': safe_str(db_sub_item.get('fringes')), 'monday_value': safe_str(col_vals.get(self.SUBITEM_FRINGE_COLUMN_ID)['text'])}, {'field': 'transaction_date', 'db_value': safe_str(db_sub_item.get('transaction_date')).split(' ')[0] if safe_str(db_sub_item.get('transaction_date')) else None, 'monday_value': safe_str(col_vals[self.SUBITEM_DATE_COLUMN_ID]['text'])}, {'field': 'due_date', 'db_value': safe_str(db_sub_item.get('due_date')).split(' ')[0] if safe_str(db_sub_item.get('due_date')) else None, 'monday_value': safe_str(col_vals[self.SUBITEM_DUE_DATE_COLUMN_ID]['text'] if isinstance(col_vals[self.SUBITEM_DUE_DATE_COLUMN_ID], dict) else col_vals[self.SUBITEM_DUE_DATE_COLUMN_ID])}]
        for f in field_map:
            if not are_values_equal(f['db_value'], f['monday_value']):
                differences.append({'field': f['field'], 'db_value': f['db_value'], 'monday_value': f['monday_value']})
        return differences
    ...
    def extract_subitem_identifiers(self, monday_sub_item):
        ...
        if project_id is not None and po_number is not None and (detail_num is not None) and (line_number is not None):
            ...
            self.logger.warning('[extract_subitem_identifiers] - Subitem missing one of the required identifiers.')
            return None
