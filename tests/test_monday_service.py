...
class TestMondayService(unittest.TestCase):
    ...
    def test__make_request_success(self, mock_post):
        ...
        mock_post.assert_called_once_with(self.service.api_url, json={'query': query, 'variables': variables}, headers={'Authorization': self.service.api_token})
    ...
    def test_update_po_status(self, mock_db_util, mock_monday_util):
        ...
        expected_variables = {'board_id': int(self.service.board_id), 'item_id': pulse_id, 'column_id': self.service.monday_util.PO_STATUS_COLUMN_ID, 'value': '{"label": "Approved"}'}
    ...
    def test_match_or_create_contact_existing_contact(self, mock_db_util, mock_monday_util):
        ...
        mock_monday_util_instance.create_item.return_value = None
    ...
    def test_match_or_create_contact_create_new_contact(self, mock_db_util, mock_monday_util):
        ...
        mock_monday_util_instance.find_contact_item_by_name.assert_called_once_with('VendorName')
        mock_monday_util_instance.create_item.assert_called_once_with(group_id='contacts_group_id', item_name='VendorName', column_values={self.service.monday_util.CONTACT_NAME: 'VendorName', self.service.monday_util.CONTACT_EMAIL: 'vendor@example.com', self.service.monday_util.CONTACT_PHONE: '123-456-7890'})
        mock_db_util_instance.link_contact_to_po.assert_called_once_with('PO123', '456')
    ...
    def test_sync_main_items_from_monday_board_success(self, mock_monday_util, mock_db_util, mock_monday_api):
        ...
        mock_monday_api_instance.fetch_all_items.return_value = [{'id': '1', 'name': 'PO1'}, {'id': '2', 'name': 'PO2'}]
        mock_db_util_instance = mock_db_util.return_value
        mock_monday_util_instance = mock_monday_util.return_value
        mock_db_util_instance.prep_main_item_event_for_db_creation.side_effect = [{'pulse_id': 1, 'po_number': 'PO123', 'state': 'Open'}, {'pulse_id': 2, 'po_number': 'PO456', 'state': 'Closed'}]
    ...
    def test_sync_main_items_from_monday_board_exception_syncing(self, mock_monday_util, mock_db_util, mock_monday_api):
        ...
        """
        mock_monday_api_instance = mock_monday_api.return_value
        mock_monday_api_instance.fetch_all_items.return_value = [{'id': '1', 'name': 'PO1'}]
        mock_db_util_instance = mock_db_util.return_value
        mock_monday_util_instance = mock_monday_util.return_value
    ...
    def test_sync_sub_items_from_monday_board_success(self, mock_monday_api):
        ...
        """
        mock_monday_api_instance = mock_monday_api.return_value
        mock_monday_api_instance.fetch_all_sub_items.return_value = [{'id': '1', 'parent_item': {'id': '10'}, 'name': 'SubItem1'}, {'id': '2', 'parent_item': {'id': '20'}, 'name': 'SubItem2'}]
        mock_db_util_instance = MagicMock()
        mock_db_util_instance.prep_sub_item_event_for_db_creation.side_effect = [{'pulse_id': 1, 'parent_id': 100, 'detail_number': 'D1', 'description': 'Desc1', 'quantity': 1.0, 'account_number_id': 1, 'is_receipt': 0}, {'pulse_id': 2, 'parent_id': 200, 'detail_number': 'D2', 'description': 'Desc2', 'quantity': 2.0, 'account_number_id': 1, 'is_receipt': 0}]
        mock_db_util_instance.create_or_update_sub_item_in_db.side_effect = [{'status': 'Created'}, {'status': 'Updated'}]
        self.service.db_util = mock_db_util_instance
        with self.assertLogs(level='INFO') as log:
    ...
    def test_sync_sub_items_from_monday_board_sync_exception(self, mock_monday_api):
        ...
        """
        mock_monday_api_instance = mock_monday_api.return_value
        mock_monday_api_instance.fetch_all_sub_items.return_value = [{'id': '1', 'parent_item': {'id': '10'}, 'name': 'SubItem1'}]
        mock_db_util_instance = MagicMock()
    ...
    def test_sync_sub_items_from_monday_board_orphan(self, mock_monday_api):
        ...
        """
        mock_monday_api_instance = mock_monday_api.return_value
        mock_monday_api_instance.fetch_all_sub_items.return_value = [{'id': '1', 'parent_item': {'id': None}, 'name': 'SubItem1'}, {'id': '2', 'parent_item': {'id': '20'}, 'name': 'SubItem2'}]
        mock_db_util_instance = MagicMock()
        mock_db_util_instance.prep_sub_item_event_for_db_creation.side_effect = [None, {'pulse_id': 2, 'parent_id': 200, 'detail_number': 'D2', 'description': 'Desc2', 'quantity': 2.0, 'account_number_id': 1, 'is_receipt': 0}]
        mock_db_util_instance.create_or_update_sub_item_in_db.side_effect = [{'status': 'Updated'}]
        self.service.db_util = mock_db_util_instance
        with self.assertLogs(level='WARNING') as log:
            self.service.sync_sub_items_from_monday_board()
        self.assertIn('Skipping sub-item with pulse_id 1 due to missing parent.', log.output[0])
        mock_db_util_instance.create_or_update_sub_item_in_db.assert_called_once_with({'pulse_id': 2, 'parent_id': 200, 'detail_number': 'D2', 'description': 'Desc2', 'quantity': 2.0, 'account_number_id': 1, 'is_receipt': 0})
    ...
    def test_sync_sub_items_from_monday_board_handle_error_in_create_update(self, mock_monday_api):
        ...
        mock_monday_api_instance.fetch_all_sub_items.return_value = [{'id': '1', 'parent_item': {'id': '10'}, 'name': 'SubItem1'}]
        mock_db_util_instance = MagicMock()
        mock_db_util_instance.prep_sub_item_event_for_db_creation.return_value = {'pulse_id': 1, 'parent_id': 100, 'detail_number': 'D1', 'description': 'Desc1', 'quantity': 1.0, 'account_number_id': 1, 'is_receipt': 0}
    ...
    def test_sync_sub_items_from_monday_board_empty_subitems(self, mock_monday_api):
        ...
        mock_db_util_instance = MagicMock()
        self.service.db_util = mock_db_util_instance
        with patch.object(self.service.db_util, 'prep_sub_item_event_for_db_creation') as mock_prep_sub, patch.object(self.service.db_util, 'create_or_update_sub_item_in_db') as mock_create_update_sub:
            self.service.sync_sub_items_from_monday_board()
        mock_monday_api_instance.fetch_all_sub_items.assert_called_once_with()
    ...
    def test_sync_contacts_from_monday_board_success(self, mock_monday_util, mock_db_util, mock_monday_api):
        ...
        """
        mock_monday_api_instance = mock_monday_api.return_value
        mock_monday_api_instance.fetch_all_contacts.return_value = [{'id': '1', 'name': 'Contact1'}, {'id': '2', 'name': 'Contact2'}]
        mock_monday_util_instance = mock_monday_util.return_value
        mock_db_util_instance = mock_db_util.return_value
        mock_monday_util_instance.prep_contact_event_for_db_creation.side_effect = [{'pulse_id': 1, 'name': 'Contact1', 'email': 'contact1@example.com'}, {'pulse_id': 2, 'name': 'Contact2', 'email': 'contact2@example.com'}]
    ...
    def test_sync_contacts_from_monday_board_sync_exception(self, mock_monday_util, mock_db_util, mock_monday_api):
        ...
        """
        mock_monday_api_instance = mock_monday_api.return_value
        mock_monday_api_instance.fetch_all_contacts.return_value = [{'id': '1', 'name': 'Contact1'}]
        mock_monday_util_instance = mock_monday_util.return_value
        mock_db_util_instance = mock_db_util.return_value
    ...
    def test_sync_contacts_from_monday_board_empty_contacts(self, mock_monday_util, mock_db_util, mock_monday_api):
        ...
        mock_db_util_instance = mock_db_util.return_value
        with patch.object(self.service.db_util, 'prep_contact_event_for_db_creation') as mock_prep_contact, patch.object(self.service.db_util, 'create_or_update_contact_item_in_db') as mock_create_or_update_contact:
            self.service.sync_contacts_from_monday_board()
        mock_monday_api_instance.fetch_all_contacts.assert_called_once_with(self.service.contact_board_id)
