...
def test_create_item(monday_api_instance):
    with patch('requests.post') as mock_post:
        ...
        mock_post.return_value.json.return_value = {'data': {'create_item': {'id': '12345'}, 'complexity': {'query': 100, 'before': 10000, 'after': 9900}}}
...
def test_fetch_all_items(monday_api_instance):
    with patch('requests.post') as mock_post:
        mock_post.return_value.raise_for_status = MagicMock()
        mock_post.return_value.json.side_effect = [{'data': {'boards': [{'items_page': {'cursor': 'next_cursor', 'items': [{'id': '1', 'name': 'Item 1', 'column_values': []}]}}], 'complexity': {'query': 50, 'before': 10000, 'after': 9950}}}, {'data': {'next_items_page': {'cursor': None, 'items': [{'id': '2', 'name': 'Item 2', 'column_values': []}]}, 'complexity': {'query': 50, 'before': 9950, 'after': 9900}}}]
...
def test_find_or_create_contact_in_monday(monday_api_instance):
    with patch.object(monday_api_instance, 'fetch_contact_by_name') as mock_fetch_contact, patch.object(monday_api_instance, 'create_contact') as mock_create_contact:
        ...
        mock_create_contact.return_value = {'data': {'create_item': {'id': '1010', 'name': 'New Contact'}}}
