...
class TestMondayUtil(unittest.TestCase):
    ...
    def test_get_subitems_column_id_success(self, mock_post):
        # Mock the API response for successful retrieval
    ...
    def test_create_item_success(self, mock_post):
        # Mock the API response for successful item creation
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'data': {
        ...
        item_id = self.monday_util.create_item(
            group_id='group_1',
            item_name='Test Item',
    ...
    def test_update_item_columns_success(self, mock_post):
        ...
        result = self.monday_util.update_item_columns(
            item_id='456',
    ...
    def test_subitem_column_values_formatter(self):
        # Test the formatter with some sample data
        ...
        expected_values = {
            'payment_notes__1': 'Payment due',
            'status4': {'label': 'Pending'},
            'text0': '123',
            'text98': 'Test Description',
            'numbers0': 2,
            'numbers9': 100.0,
            'date': {'date': '2023-10-01'},
            'date_1__1': {'date': '2023-10-15'},
            'dropdown': {'ids': ['2']},
            'link': {'url': 'http://example.com', 'text': 'Link'}
        }
        self.assertEqual(column_values, expected_values)

    @patch('requests.post')
    def test_create_subitem_success(self, mock_post):
        # Mock the API response for successful subitem creation
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'data': {
        ...
        subitem_id = self.monday_util.create_subitem(
            parent_item_id='456',
    ...
    def test_update_subitem_columns_success(self, mock_post):
        # Mock the API response for successful subitem update
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'data': {
        ...
        result = self.monday_util.update_subitem_columns(
            subitem_id='789',
            column_values={'status4': {'label': 'Completed'}}
        )
        self.assertTrue(result)
    ...
    def test_link_contact_to_po_item_success(self, mock_post):
        # Mock the API response for successful linking
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'data': {
        ...
        result = self.monday_util.link_contact_to_po_item(
            po_item_id='456',
            contact_item_id='123'
        )
        self.assertTrue(result)
