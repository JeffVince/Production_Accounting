...
class DropboxClient(metaclass=SingletonMeta):
    ...
    def load_access_token(self):
        ...
        try:
            ...
            self.logger.info("Access token file not found.")
            return None
    ...
    def list_root_folder(self):
        ...
        try:
            ...
            self.logger.info("Root folder contents:")
            for entry in result.entries:
                self.logger.info(f" - {entry.name}")
        except Exception as e:
            self.logger.error(f"Error listing root folder: {e}", exc_info=True)
    ...
    def list_folder_changes(self, cursor):
        ...
        with self._internal_lock:
            ...
            try:
                ...
                self.logger.error(f"Error listing folder changes: {e}", exc_info=True)
                return [], cursor
    ...
    def load_cursor(self):
        ...
        try:
            with open(cursor_file, 'r') as f:
                ...
                self.logger.debug(f"Loaded cursor for member {self.member_id}: {cursor}")
                return cursor
        except Exception as e:
            self.logger.error(f"Error loading cursor for member {self.member_id}: {e}", exc_info=True)
            return None
    ...
    def save_cursor(self, cursor):
        ...
        try:
            ...
            self.logger.debug(f"Saved cursor for member {self.member_id}: {cursor}")
        except Exception as e:
            self.logger.error(f"Error saving cursor for member {self.member_id}: {e}", exc_info=True)
            ...
            if time_to_refresh > 0:
                ...
                try:
                    ...
                    if self.namespace_id:
                        path_root = common.PathRoot.namespace_id(self.namespace_id)
                        self.dbx = self.dbx.with_path_root(path_root)
                    self.logger.info("Access token refreshed and Dropbox client updated.")
                except Exception as e:
