...
from files_po_log.po_log_processor import POLogProcessor
...
class DropboxService(metaclass=SingletonMeta):
    ...
    def __init__(self):
        ...
        if not hasattr(self, '_initialized'):
            ...
            self.logger.info('[__init__] - 📦 Dropbox Service initialized 🌟')
    ...
    def determine_file_type(self, path: str):
        ...
        """
        self.logger.info(f'[determine_file_type] - 🔍 Checking file type for: {self.dropbox_util.get_last_path_component_generic(path)}')
        ...
        try:
            if self.PO_LOG_FOLDER_NAME in path:
                ...
                if project_number_match:
                    ...
                    self.logger.info(f'[determine_file_type] - 🗂 Identified as PO Log for Project ID {project_number}')
                    return self.po_log_orchestrator(path)
                else:
                    self.logger.warning(f"[determine_file_type] - ⚠️ Filename '{filename}' does not match PO Log format.")
                    return
            if re.search(self.INVOICE_REGEX, filename, re.IGNORECASE):
                self.logger.info(f'[determine_file_type] - 💰 Identified as invoice: {filename}')
                return self.process_invoice(path)
            if re.search(self.TAX_FORM_REGEX, filename, re.IGNORECASE):
                self.logger.info(f'[determine_file_type] - 💼 Identified as tax form: {filename}')
                return self.process_tax_form(path)
            if re.search(self.RECEIPT_REGEX, filename, re.IGNORECASE):
                self.logger.info(f'[determine_file_type] - 🧾 Identified as receipt: {filename}')
                return self.process_receipt(path)
            if re.search(self.SHOWBIZ_REGEX, filename, re.IGNORECASE):
                self.logger.info(f'[determine_file_type] - 📑 Identified as budget file: {filename}')
                return None
            self.logger.debug(f'[determine_file_type] - ❌ Unsupported file type: {filename}')
            return None
        except Exception as e:
            self.logger.exception(f'[determine_file_type] - 💥 Error determining file type for {filename}: {e}', exc_info=True)
            return None

    def process_budget(self, dropbox_path: str):
        self.logger.info(f'[process_budget] - 💼 Processing budget: {dropbox_path}')
        filename = os.path.basename(dropbox_path)
        try:
            if not filename.endswith('.mbb') or filename.endswith('.mbb.lck'):
                self.logger.info('[process_budget] - ❌ Not a valid .mbb file.')
                return
        except Exception as e:
            self.logger.exception(f'[process_budget] - 💥 Error checking extension: {e}', exc_info=True)
            return
        try:
            segments = dropbox_path.strip('/').split('/')
            if len(segments) < 4:
                self.logger.info('[process_budget] - ❌ Not enough path segments.')
                return
            ...
            if budget_folder != '5. Budget' or phase_folder not in ['1.2 Working', '1.3 Actuals']:
                self.logger.info('[process_budget] - ❌ Budget file not in correct folder.')
                return
            project_number_match = re.match('^\\d{4}', project_folder)
            if not project_number_match:
                self.logger.info("[process_budget] - ❌ Can't determine Project ID.")
                return
            project_number = project_number_match.group()
            self.logger.info(f'[process_budget] - 🔑 Project ID: {project_number}')
        except Exception as e:
            self.logger.exception(f'[process_budget] - 💥 Error parsing path: {e}', exc_info=True)
            return
        try:
            budget_root = '/'.join(segments[0:3])
            po_logs_path = f'/{budget_root}/1.5 PO Logs'
            self.logger.info(f'[process_budget] - 🗂 PO Logs folder: {po_logs_path}')
        except Exception as e:
            self.logger.exception(f'[process_budget] - 💥 Error determining PO Logs folder: {e}', exc_info=True)
            return
        import requests
        server_url = 'http://localhost:5004/enqueue'
        self.logger.info('[process_budget] - 🖨 Triggering ShowbizPoLogPrinter via server with file URL...')
        try:
            response = requests.post(server_url, json={'project_number': project_number, 'file_path': dropbox_path}, timeout=10)
            if response.status_code == 200:
                job_id = response.json().get('job_id')
                self.logger.info(f'[process_budget] - 🎉 Triggered server job with job_id: {job_id}')
            else:
                self.logger.error(f'[process_budget] - ❌ Failed to trigger server job. Status: {response.status_code}, Response: {response.text}')
                return
        except Exception as e:
            self.logger.exception(f'[process_budget] - 💥 Error triggering server job: {e}', exc_info=True)
            return
        self.logger.info('[process_budget] - ✅ process_budget completed successfully, server job triggered with file URL.')
    ...
    def po_log_orchestrator(self, path: str):
        ...
        """
        self.logger.info(f'[po_log_orchestrator] - 📝 Processing PO log: {path}')
        ...
        self.logger.info('[po_log_orchestrator] - 🔧 DB and Monday processing dispatched to background threads.')
    ...
    def extract_data_from_po_log(self, temp_file_path: str, project_number: str):
        ...
        try:
            ...
            self.logger.info(f'[extract_data_from_po_log] - 📝 Parsed PO Log for project {project_number}: {len(main_items)} main items, {len(detail_items)} detail items, {len(contacts)} contacts.')
            return (main_items, detail_items, contacts)
        except Exception as e:
            self.logger.exception(f'[extract_data_from_po_log] - 💥 Failed to parse PO Log: {e}', exc_info=True)
            return ([], [], [])
    ...
    def add_po_data_to_db(self, main_items, detail_items, contacts, project_number: str):
        ...
        *New Batch Logic*:
        ...
        """
        self.logger.info(f'[add_po_data_to_db] - 🔧 [NEW BATCH LOGIC] Processing project_number={project_number} with batch queries.')
        pn_int = int(project_number)
        all_db_contacts = self.database_util.search_contacts()
        self.logger.info(f'[add_po_data_to_db] - ✅ Retrieved {(len(all_db_contacts) if isinstance(all_db_contacts, list) else 1)} contacts.')
        ...
        self.logger.info(f'[add_po_data_to_db] - ✅ Found {len(existing_pos)} existing POs for project_number={pn_int}.')
            ...
            try:
                ...
                if existing_po:
                    if _po_has_changes(existing_po, item, contact_id):
                        self.logger.info(f'[add_po_data_to_db] - 🔄 Updating existing PO {po_number} for project {pn_int}.')
                        updated_po = self.database_util.update_purchase_order_by_keys(project_number=pn_int, po_number=po_number, description=item.get('description'), po_type=item.get('po_type'), contact_id=contact_id)
                        if updated_po:
                            pos_by_number[po_number] = updated_po
                    else:
                        self.logger.debug(f'[add_po_data_to_db] - ⏭ No changes detected for existing PO {po_number}. Skipping update.')
                else:
                    self.logger.info(f'[add_po_data_to_db] - 🆕 Creating new PO {po_number} for project {pn_int}.')
                    new_po = self.database_util.create_purchase_order_by_keys(project_number=pn_int, po_number=po_number, description=item.get('description'), po_type=item.get('po_type'), contact_id=contact_id)
                    if new_po:
                        pos_by_number[po_number] = new_po
            except Exception as ex:
                self.logger.error(f"[add_po_data_to_db] - 💥 Error processing PO creation/update for po_number={item.get('po_number')}: {ex}", exc_info=True)
        ...
        for d in existing_details:
            ...
            detail_dict[key] = d
        self.logger.info(f'[add_po_data_to_db] - ✅ Found {len(existing_details)} existing DetailItems for project_number={pn_int}.')
            ...
            try:
                ...
                if existing_di:
                    ...
                    if current_state in COMPLETED_STATUSES:
                        self.logger.info(f"[add_po_data_to_db] - ⏭ DetailItem {key} is in final state '{current_state}'. Skipping update.")
                        continue
                    if _detail_item_has_changes(existing_di, sub_item):
                        self.logger.info(f'[add_po_data_to_db] - 🔄 Updating existing DetailItem {key} in the database.')
                        updated_di = self.database_util.update_detail_item_by_keys(project_number=pn_int, po_number=po_number, detail_number=detail_number, line_number=line_number, vendor=sub_item.get('vendor'), description=sub_item.get('description'), transaction_date=sub_item.get('date'), due_date=sub_item.get('due date'), rate=sub_item.get('rate', 0), quantity=sub_item.get('quantity', 1), ot=sub_item.get('OT', 0), fringes=sub_item.get('fringes', 0), state=sub_item['state'], account_code=sub_item['account'], payment_type=sub_item['payment_type'])
                        if updated_di:
                            detail_dict[key] = updated_di
                    else:
                        self.logger.debug(f'[add_po_data_to_db] - ⏭ No changes detected for existing DetailItem {key}. Skipping update.')
                else:
                    self.logger.debug(f'[add_po_data_to_db] - 🆕 Creating detail item {key}')
                    new_di = self.database_util.create_detail_item_by_keys(project_number=pn_int, po_number=po_number, detail_number=detail_number, line_number=line_number, vendor=sub_item.get('vendor'), description=sub_item.get('description'), transaction_date=sub_item.get('date'), due_date=sub_item.get('due date'), rate=sub_item.get('rate', 0), quantity=sub_item.get('quantity', 1), ot=sub_item.get('OT', 0), fringes=sub_item.get('fringes', 0), state=sub_item['state'], account_code=sub_item['account'], payment_type=sub_item['payment_type'])
                    if new_di:
                        detail_dict[key] = new_di
            except Exception as ex:
                self.logger.error(f'[add_po_data_to_db] - 💥 Error processing DetailItem creation/update for PO={po_number}, detail={detail_number}, line={line_number}: {ex}', exc_info=True)
        self.logger.info('[add_po_data_to_db] - ✅ [NEW BATCH LOGIC] Completed batch PO & DetailItem processing.')
        return main_items
    ...
    def _find_or_create_contact(self, item, contacts, index, all_db_contacts):
        ...
        if not contact_name:
            return None
        self.logger.info(f'[_find_or_create_contact] - 🔍 Checking for Contact: {contact_name}')
        ...
        if contact_search:
            ...
            self.logger.info(f'[_find_or_create_contact] - 🆕 Creating Contact: {contact_name}')
    ...
    def process_invoice(self, dropbox_path: str):
        ...
        """
        self.logger.info(f'[process_invoice] - 📄 Processing invoice: {dropbox_path}')
        filename = os.path.basename(dropbox_path)
        try:
            match = re.match('^(\\d{4})_(\\d{1,2})(?:_(\\d{1,2}))?', filename)
            if not match:
                self.logger.warning(f"[process_invoice] - ⚠️ Invoice filename '{filename}' doesn't match the expected pattern.")
                return
        ...
        try:
            ...
            self.logger.info(f'[process_invoice] - 🔗 Obtained Dropbox link for invoice: {file_share_link}')
        except Exception as e:
            self.logger.exception(f'[process_invoice] - 💥 Error retrieving Dropbox link: {e}', exc_info=True)
        ...
        if not self.download_file_from_dropbox(dropbox_path, temp_file_path):
            self.logger.error(f'[process_invoice] - ❌ Could not download invoice file: {dropbox_path}')
            return
        (transaction_date, term, total) = (None, 30, 0.0)
        try:
            self.logger.info('[process_invoice] - 🔎 Scanning invoice file for data with OCRService + OpenAI...')
            ...
            if err or not info:
                self.logger.warning(f'[process_invoice] - ❌ Could not parse invoice data from OpenAI. Using defaults. Error: {err}')
            else:
            ...
            self.logger.exception(f'[process_invoice] - 💥 Error extracting invoice data: {e}', exc_info=True)
        try:
            existing_invoice = self.database_util.search_invoice_by_keys(project_number=str(project_number), po_number=str(po_number), invoice_number=str(invoice_number))
            if existing_invoice is None:
                self.logger.info(f'[process_invoice] - 🆕 Creating new invoice #{invoice_number} for PO {po_number} (project {project_number}).')
                new_invoice = self.database_util.create_invoice(project_number=project_number, po_number=po_number, invoice_number=invoice_number, transaction_date=transaction_date, term=term, total=total, file_link=file_share_link)
                ...
                self.logger.info(f'[process_invoice] - 🔄 Updating existing invoice #{invoice_number} for PO {po_number}.')
                self.database_util.update_invoice(invoice_id=invoice_id, transaction_date=transaction_date, term=term, total=total, file_link=file_share_link)
        except Exception as e:
            self.logger.exception(f'[process_invoice] - 💥 Error creating/updating invoice in DB: {e}', exc_info=True)
            self.cleanup_temp_file(temp_file_path)
            return
        self.cleanup_temp_file(temp_file_path)
        self.logger.info(f'[process_invoice] - ✅ Minimal invoice processing complete for: {dropbox_path}')
    ...
    def process_receipt(self, dropbox_path: str):
        ...
        """
        self.logger.info(f'[process_receipt] - 🧾 Processing receipt: {dropbox_path}')
        ...
        if not match:
            self.logger.warning(f"[process_receipt] - ❌ Filename '{filename}' does not match receipt pattern.")
            return
        project_number_str = match.group(1)
        ...
        try:
            ...
            if not success:
                self.logger.warning(f'[process_receipt] - 🛑 Failed to download receipt: {filename}')
                return
            ...
            if file_ext == 'pdf':
                ...
                if not extracted_text.strip():
                    self.logger.info('[process_receipt] - PDF extraction found no text; falling back to OCR.')
                    extracted_text = self._extract_text_from_pdf_with_ocr(file_data)
            else:
                extracted_text = self._extract_text_via_ocr(file_data)
            parse_failed = False
            if not extracted_text.strip():
                self.logger.warning(f'[process_receipt] - 🛑 No text extracted from receipt: {filename}. Will mark as ISSUE but continue.')
                parse_failed = True
            ...
            if not parse_failed:
                ...
                if not receipt_info:
                    self.logger.warning(f'[process_receipt] - 🛑 Could not parse receipt info from AI for: {filename} - marking as ISSUE.')
                    parse_failed = True
            else:
                self.logger.warning(f'[process_receipt] - Skipping AI parsing because no text was extracted for {filename}.')
                receipt_info = {}
            ...
            try:
                ...
                self.logger.warning(f'[process_receipt] - ❌ Could not generate shareable link for {dropbox_path}: {e}')
                file_link = None
            existing_detail = self.database_util.search_detail_item_by_keys(project_number=str(project_number), po_number=po_number, detail_number=detail_number, line_number=line_number_number)
            if not existing_detail:
                self.logger.warning(f'[process_receipt] - ❗ No matching detail item found for project={project_number}, PO={po_number}, detail={detail_number}, line={line_number_number}.')
                self.cleanup_temp_file(temp_file_path)
                return
            elif isinstance(existing_detail, list):
                existing_detail = existing_detail[0]
            spend_money_id = 1
            existing_receipts = self.database_util.search_receipts(['project_number', 'po_number', 'detail_number', 'line_number'], [project_number, po_number, detail_number, line_number_number])
            if not existing_receipts:
                new_receipt = self.database_util.create_receipt(project_number=project_number, po_number=po_number, detail_number=detail_number, line_number=line_number_number, spend_money_id=spend_money_id, total=total_amount, purchase_date=purchase_date, receipt_description=short_description, file_link=file_link)
                receipt_id = new_receipt['id'] if new_receipt else None
                self.logger.info(f'[process_receipt] - 🔄 Created new receipt with ID={receipt_id} for detail_number={detail_number}')
            else:
                existing_receipt = existing_receipts[0] if isinstance(existing_receipts, list) else existing_receipts
                receipt_id = existing_receipt['id']
                updated_receipt = self.database_util.update_receipt_by_keys(project_number=project_number, po_number=po_number, detail_number=detail_number, line_number=line_number_number, total=total_amount, purchase_date=purchase_date, receipt_description=short_description, file_link=file_link)
                self.logger.info(f'[process_receipt] - ✏️ Updated existing receipt with ID={receipt_id} for detail_number={detail_number}')
            state = 'PENDING'
            detail_subtotal = existing_detail.get('sub_total', 0.0)
            if not existing_detail['state'] == 'RECONCILED':
                if parse_failed:
                    state = 'ISSUE'
                    self.logger.info(f'[process_receipt] - Marking this receipt as ISSUE because parsing failed for {filename}.')
                else:
                    try:
                        if float(total_amount) == float(detail_subtotal):
                            state = 'REVIEWED'
                            self.logger.info('[process_receipt] - Receipt total matches the detail item subtotal. Setting state to REVIEWED.')
                        else:
                            self.logger.info('[process_receipt] - Receipt total does not match the detail item subtotal. Setting state to PO MISMATCH.')
                            state = 'PO MISMATCH'
                    except Exception as e:
                        state = 'ISSUE'
            else:
                state = 'RECONCILED'
            self.database_util.update_detail_item_by_keys(project_number=project_number, po_number=po_number, detail_number=detail_number, line_number=line_number_number, state=state, receipt_id=receipt_id)
            self.cleanup_temp_file(temp_file_path)
            self.logger.info(f'[process_receipt] - ✅ Receipt succesfully captured: {dropbox_path}')
        except Exception as e:
    ...
    def process_tax_form(self, dropbox_path: str):
        ...
        """
        self.logger.info(f'[process_tax_form] - 🗂 Processing Tax Form: {dropbox_path}')
        pass
    ...
    def folder_tax_conact_trigger(self, fut):
        ...
        try:
            ...
            if self.GET_FOLDER_LINKS:
                self.logger.info('[folder_tax_conact_trigger] - SYNCING PO FOLDER LINKS')
                ...
                def get_folder_links(processed_items):
                    for item in processed_items:
                        ...
                        if db_po['folder_link']:
                            self.logger.debug(f"[get_folder_links] - [folder_tax_conact_trigger] - ✅Folder Link is already present for {db_project['project_number']}_{db_po['po_number']}")
                        else:
                            self.logger.debug(f'[get_folder_links] - [folder_tax_conact_trigger] - Folder link not found in DB -- retrieving from Dropbox')
                ...
                def get_tax_links(processed_items):
                    for (idx, item) in enumerate(processed_items):
                        ...
                        if db_contact and db_contact['vendor_type'] == 'Vendor':
                            ...
                            if not tax_link:
                                self.logger.debug(f"[get_tax_links] - [folder_tax_conact_trigger] - Database is missing the tax form link for Contact {db_contact['id']}. Fetching a new link from Dropbox...")
                                tax_link = self.update_po_tax_form_links(item['project_number'], item['po_number'])
                                if tax_link:
                                    self.monday_api.update_monday_tax_form_link(db_contact['pulse_id'], tax_link)
                                else:
                                    self.logger.error('[get_tax_links] - [folder_tax_conact_trigger] - ❌ - Unable to get link from contact')
                            else:
                                self.logger.debug(f"[get_tax_links] - [folder_tax_conact_trigger] - ✅ Tax Link is already present for {db_contact['name']}")
            ...
            if processed_items:
                ...
                if project_number:
                    self.logger.info(f'[folder_tax_conact_trigger] - Both folder links and contact sync done. Now creating POs in Monday for project_id={project_number}.')
                    if self.ADD_PO_TO_MONDAY:
                        self.create_pos_in_monday(int(project_number))
                    else:
                        self.logger.info('[folder_tax_conact_trigger] - SKIPPING MONDAY PO CREATION')
                else:
                    self.logger.warning('[folder_tax_conact_trigger] - ❌ Could not determine project_id from processed_items.')
            else:
                self.logger.warning('[folder_tax_conact_trigger] - ❌ processed_items is empty, skipping create_pos_in_monday.')
        except Exception as e:
            self.logger.error(f'[folder_tax_conact_trigger] - ❌ after_db_done encountered an error: {e}', exc_info=True)
    ...
    def update_po_folder_link(self, project_number, po_number):
        ...
        try:
            ...
            if not po_data or not len(po_data) > 0:
                logger.warning(f'❌ No PO data returned for PO={project_number}_{str(po_number).zfill(2)}, aborting link updates.')
                return
            po_data = po_data
            ...
            if not project_item or len(project_item) < 1:
                logger.warning(f'⚠️ Could not determine project folder name for {project_number}, no links will be found.')
                return
    ...
    def update_po_tax_form_links(self, project_number, po_number):
        ...
        try:
            ...
            self.logger.info(f'[update_po_tax_form_links] - 📑 Updated tax form link for PO {project_number}_{po_number} => {new_tax_form_link}')
            return new_tax_form_link
        except Exception as e:
            self.logger.error(f'[update_po_tax_form_links] - 💥 Could not update PO tax form link for {project_number}_{po_number}: {e}', exc_info=True)
    ...
    def create_pos_in_monday(self, project_number):
        ...
        """
        self.logger.info('[create_pos_in_monday] - 🌐 Processing PO data in Monday.com...')
        ...
        for db_item in processed_items:
            ...
            column_values_str = self.monday_util.po_column_values_formatter(project_id=str(project_number), po_number=db_item['po_number'], description=db_item.get('description'), contact_pulse_id=db_item['contact_pulse_id'], folder_link=db_item.get('folder_link'), producer_id=None, name=db_item['contact_name'])
            ...
            if key in monday_items_map:
                ...
                if differences:
                    self.logger.debug(f'[create_pos_in_monday] - Item differs for PO {po_no}. Differences: {differences}')
                    items_to_update.append({'db_item': db_item, 'column_values': new_vals, 'monday_item_id': monday_item['id']})
                else:
                    self.logger.debug(f'[create_pos_in_monday] - No changes for PO {po_no}, skipping update.')
            else:
                items_to_create.append({'db_item': db_item, 'column_values': new_vals, 'monday_item_id': None})
        if items_to_create:
            self.logger.info(f'[create_pos_in_monday] - 🆕 Need to create {len(items_to_create)} main items on Monday.')
            created_mapping = self.monday_api.batch_create_or_update_items(items_to_create, project_id=project_number, create=True)
            for itm in created_mapping:
                ...
                monday_items_map[p, po] = {'id': monday_item_id, 'name': f'PO #{po}', 'column_values': itm['column_values']}
        if items_to_update:
            self.logger.info(f'[create_pos_in_monday] - ✏️ Need to update {len(items_to_update)} main items on Monday.')
            updated_mapping = self.monday_api.batch_create_or_update_items(items_to_update, project_id=project_number, create=False)
        ...
        for db_item in processed_items:
            ...
            if main_monday_item and (not db_item.get('pulse_id')):
                ...
                if updated:
                    db_item['pulse_id'] = monday_item_id
                    self.logger.info(f'[create_pos_in_monday] - 🗂 PO {po_no} now has pulse_id {monday_item_id} in DB')
        for db_item in processed_items:
            p_id = project_number
            po_no = int(db_item['po_number'])
            main_monday_item = monday_items_map.get((p_id, po_no))
            if not main_monday_item:
                self.logger.warning(f'[create_pos_in_monday] - ❌ No Monday main item found for PO {po_no}, skipping subitems.')
                continue
            ...
            for sdb in sub_items_db:
                ...
                if db_item['po_type'] in ['PC', 'CC']:
                    ...
                    if receipt_id:
                        ...
                        existing_receipts = self.database_util.search_receipts(['project_number', 'po_number', 'detail_number', 'line_number'], [project_number, int(po_no), sdb['detail_number'], sdb['line_number']])
                        if existing_receipts:
                            ...
                            self.database_util.update_detail_item_by_keys(project_number=p_id, po_number=db_item['po_number'], detail_number=sdb['detail_number'], line_number=sdb['line_number'], receipt_id=rid)
                            sdb['receipt_id'] = rid
                    ...
                    if invoice_id:
                        ...
                        existing_invoices = self.database_util.search_invoices(['project_number', 'po_number', 'invoice_number'], [project_number, int(po_no), sdb['detail_number']])
                        if existing_invoices:
                            ...
                            self.database_util.update_detail_item_by_keys(project_number=p_id, po_number=str(po_no), detail_number=sdb['detail_number'], line_number=sdb['line_number'], invoice_id=inv_id)
                            sdb['invoice_id'] = inv_id
                sub_col_values_str = self.monday_util.subitem_column_values_formatter(project_id=project_number, po_number=db_item['po_number'], detail_number=sdb['detail_number'], line_number=sdb['line_number'], status=sdb.get('state'), description=sdb.get('description'), quantity=sdb.get('quantity'), rate=sdb.get('rate'), date=sdb.get('transaction_date'), due_date=sdb.get('due_date'), account_number=sdb['account_code'], link=file_link_for_subitem, OT=sdb.get('ot'), fringes=sdb.get('fringes'))
                ...
                if sub_key in global_subitem_map:
                    ...
                    if differences:
                        self.logger.debug(f"[create_pos_in_monday] - Sub-item differs for detail #{sdb['detail_number']} (PO {po_no}). {differences}")
                        subitems_to_update.append({'db_sub_item': sdb, 'column_values': new_sub_vals, 'parent_id': main_monday_id, 'monday_item_id': msub['id']})
                    else:
                        self.logger.debug(f"[create_pos_in_monday] - No changes for detail #{sdb['detail_number']} (PO {po_no}).")
                        ...
                        if current_pulse_id != int(sub_pulse_id) or current_parent_id != int(main_monday_id):
                            self.logger.debug(f"[create_pos_in_monday] - Pulse mismatch for detail #{sdb['detail_number']} (PO {po_no}). DB has pulse_id={current_pulse_id}, parent_pulse_id={current_parent_id}; Monday is {sub_pulse_id}, {main_monday_id}.")
                            self.database_util.update_detail_item_by_keys(project_number=project_number, po_number=db_item['po_number'], detail_number=sdb['detail_number'], line_number=sdb['line_number'], pulse_id=sub_pulse_id, parent_pulse_id=main_monday_id)
                            sdb['pulse_id'] = sub_pulse_id
                            sdb['parent_pulse_id'] = main_monday_id
                        else:
                            self.logger.debug(f"[create_pos_in_monday] - Pulse IDs already match for detail #{sdb['detail_number']} (PO {po_no}), no DB update needed.")
                else:
                    subitems_to_create.append({'db_sub_item': sdb, 'column_values': new_sub_vals, 'parent_id': main_monday_id})
            if subitems_to_create:
                self.logger.info(f'[create_pos_in_monday] - 🆕 Need to create {len(subitems_to_create)} sub-items for PO {po_no}.')
                self._batch_create_subitems(subitems_to_create, main_monday_id, project_number, db_item)
            if subitems_to_update:
                self.logger.info(f'[create_pos_in_monday] - ✏️ Need to update {len(subitems_to_update)} sub-items for PO {po_no}.')
                self._batch_update_subitems(subitems_to_update, main_monday_id, project_number, db_item)
        self.logger.info('[create_pos_in_monday] - ✅ Monday.com processing of PO data complete.')
    ...
    def _batch_create_subitems(self, subitems_to_create, parent_item_id, project_number, db_item):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for (idx, chunk) in enumerate(create_chunks):
                future = executor.submit(self.monday_api.batch_create_or_update_subitems, chunk, parent_item_id=parent_item_id, create=True)
                future_to_index[future] = idx
            for future in as_completed(future_to_index):
                idx = future_to_index[future]
                try:
                    chunk_result = future.result()
                    self.logger.debug(f'[_batch_create_subitems] - Subitems create-chunk #{idx + 1} completed.')
        ...
        for csub in all_created_subs:
            ...
            self.database_util.update_detail_item_by_keys(project_number, db_item['po_number'], db_sub_item['detail_number'], db_sub_item['line_number'], pulse_id=monday_subitem_id, parent_pulse_id=parent_item_id)
    ...
    def _batch_update_subitems(self, subitems_to_update, parent_item_id, project_number, db_item):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for (idx, chunk) in enumerate(update_chunks):
                future = executor.submit(self.monday_api.batch_create_or_update_subitems, chunk, parent_item_id=parent_item_id, create=False)
                future_to_index[future] = idx
            for future in as_completed(future_to_index):
                idx = future_to_index[future]
                try:
                    chunk_result = future.result()
                    self.logger.debug(f'[_batch_update_subitems] - Subitems update-chunk #{idx + 1} completed.')
    ...
    def download_file_from_dropbox(self, path: str, temp_file_path: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f'[download_file_from_dropbox] - 📂 File saved to {temp_file_path}')
            return True
        except Exception as e:
            self.logger.exception(f'[download_file_from_dropbox] - 💥 Failed to download file: {e}', exc_info=True)
            return False
    ...
    def _parse_tax_number(self, tax_str: str) -> Optional[int]:
        ...
        try:
            ...
            self.logger.warning(f"[_parse_tax_number] - ⚠️ Could not parse tax number '{tax_str}' as integer after removing hyphens.")
            return None
    ...
    def cleanup_temp_file(self, temp_file_path: str):
        ...
        try:
            if os.path.exists(temp_file_path):
                ...
                self.logger.info(f'[cleanup_temp_file] - 🧹 Cleaned up temp file {temp_file_path}')
        except Exception as e:
            self.logger.warning(f'[cleanup_temp_file] - ⚠️ Could not remove temp file: {e}')
    ...
    def _extract_text_from_pdf(self, file_data: bytes) -> str:
        ...
        self.logger.debug(f'[_extract_text_from_pdf] - _extract_text_from_pdf: PDF data length={len(file_data)} bytes.')
        try:
            pdf_reader = PyPDF2.PdfReader(BytesIO(file_data))
            self.logger.debug(f'[_extract_text_from_pdf] - PyPDF2 sees {len(pdf_reader.pages)} page(s).')
            text_chunks = []
            for (idx, page) in enumerate(pdf_reader.pages, start=1):
                page_text = page.extract_text() or ''
                if page_text:
                    self.logger.debug(f'[_extract_text_from_pdf] - PyPDF2 page {idx}: extracted {len(page_text)} chars.')
                    text_chunks.append(page_text)
                else:
                    self.logger.debug(f'[_extract_text_from_pdf] - PyPDF2 page {idx}: no text found.')
            extracted_text = '\n'.join(text_chunks)
            self.logger.debug(f'[_extract_text_from_pdf] - PyPDF2 total text length={len(extracted_text)}.')
            if len(extracted_text.strip()) < 20:
                self.logger.info(f'[_extract_text_from_pdf] - PyPDF2 extracted fewer than 20 chars ({len(extracted_text.strip())}), treating as no text found.')
                extracted_text = ''
            if extracted_text.strip():
                return extracted_text
            self.logger.info('[_extract_text_from_pdf] - No or insufficient text from PyPDF2; extracting images via PyMuPDF...')
            pdf_document = fitz.open(stream=file_data, filetype='pdf')
            embedded_ocr_results = []
            for page_idx in range(pdf_document.page_count):
                page = pdf_document[page_idx]
                images = page.get_images(full=True)
                self.logger.debug(f'[_extract_text_from_pdf] - Page {page_idx + 1}: found {len(images)} images.')
                for (img_ix, img_info) in enumerate(images, start=1):
                    ...
                    try:
                        ...
                        self.logger.debug(f'[_extract_text_from_pdf] - Image {img_ix}: OCR extracted {len(text_in_image)} chars.')
                        embedded_ocr_results.append(text_in_image)
                    except Exception as e:
                        self.logger.warning(f'[_extract_text_from_pdf] - Could not OCR an embedded PDF image: {e}')
            fallback_text = '\n'.join(embedded_ocr_results)
            self.logger.info(f'[_extract_text_from_pdf] - PyMuPDF fallback extracted {len(fallback_text)} chars in total.')
            return fallback_text
        except Exception as e:
            self.logger.warning(f'[_extract_text_from_pdf] - Could not parse PDF with PyPDF2 or PyMuPDF: {e}')
            return ''
    ...
    def _extract_text_via_ocr(self, file_data: bytes) -> str:
        try:
            ...
            self.logger.warning(f'[_extract_text_via_ocr] - OCR extraction failed: {e}')
            return ''
    ...
    def scan_project_receipts(self, project_number: str):
        ...
        """
        self.logger.info(f'[scan_project_receipts] - 🔎 Starting receipt scan for project_number={project_number}')
        project_folder_path = self.dropbox_api.find_project_folder(project_number, namespace='2024')
        if not project_folder_path:
            self.logger.warning(f"[scan_project_receipts] - ❌ Could not find a matching project folder for '{project_number}' under 2024.")
            return
        self.logger.info(f'[scan_project_receipts] - 📂 Project folder resolved: {project_folder_path}')
        ...
        self.logger.info(f'[scan_project_receipts] - ✅ Finished scanning receipts for project_number={project_number}.')
    ...
    def _scan_and_process_receipts_in_folder(self, folder_path: str, project_number: str):
        ...
        if not entries:
            self.logger.debug(f"[_scan_and_process_receipts_in_folder] - 📂 No entries found under '{folder_path}'")
            return
        for entry in entries:
            ...
            if re.search(self.RECEIPT_REGEX, file_name, re.IGNORECASE):
                self.logger.debug(f'[_scan_and_process_receipts_in_folder] - 🧾 Potential receipt found: {dropbox_path}')
        ...
        try:
            ...
            for e in entries:
                if isinstance(e, files.FolderMetadata):
                    results.append({'name': e.name, 'path_lower': e.path_lower, 'path_display': e.path_display, 'is_folder': True})
                elif isinstance(e, files.FileMetadata):
                    results.append({'name': e.name, 'path_lower': e.path_lower, 'path_display': e.path_display, 'is_folder': False})
        except Exception as ex:
            self.logger.warning(f'[_list_folder_recursive] - ⚠️ Could not list folder recursively: {folder_path}, Error: {ex}')
        return results
    ...
    def _scan_po_folder_for_invoices(self, folder_path: str, project_number: str, folder_po_number: str):
        ...
        for entry in entries:
            ...
            if re.search(self.INVOICE_REGEX, file_name, re.IGNORECASE):
                self.logger.debug(f'[_scan_po_folder_for_invoices] - 📄 Potential invoice found in {folder_path}: {dropbox_path}')
