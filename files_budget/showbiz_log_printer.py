...
from files_dropbox.dropbox_api import dropbox_api  # Adjust as needed
...
def bring_app_to_front(app_name):
    ...
    if app:
        ...
        time.sleep(1)  # Give time to bring it to front
    else:
...
class ShowbizPoLogPrinter:
    ...
    def setup_images(self):
        ...
        self.logger.info("🖼️ Image paths initialized successfully.")
    #endregion
    ...
    def is_file_open_by_window_title(self, file_name, retries=3, delay=1):
        ...
        for attempt in range(retries):
            try:
                ...
                self.logger.error(f"❌ Attempt {attempt+1}/{retries} to find window: {e}")
            time.sleep(delay)
    ...
    def click_button(self, image_path, confidence=0.8):
        ...
        if location:
            ...
            # Save preview button position for later
                ...
                if desired_state:
                    ...
                    if location_checked:
                        ...
                        if location_unchecked:
                            ...
                            pyautogui.moveTo(x, y, duration=0.1)
                            pyautogui.click()
                            self.logger.info(f"🔘 Toggled '{checkbox_name}' to checked.")
                            time.sleep(1)
                        else:
                            self.logger.warning(f"⚠️ Unchecked image for '{checkbox_name}' not found on attempt {attempt}.")
                else:
                    # Want it unchecked
                    location_unchecked = pyautogui.locateOnScreen(unchecked_image_path, confidence=0.97)
                    if location_unchecked:
                        ...
                        # It might be checked, try clicking the checked image
                        ...
                        if location_checked:
                            ...
                            pyautogui.moveTo(x, y, duration=0.1)
                            pyautogui.click()
                            self.logger.info(f"🔘 Toggled '{checkbox_name}' to unchecked.")
                            time.sleep(1)
                        else:
                            self.logger.warning(f"⚠️ Checked image for '{checkbox_name}' not found on attempt {attempt}.")

            except Exception as e:
                self.logger.error(f"❌ Error ensuring checkbox '{checkbox_name}' state: {e}")
    ...
    def ensure_all_settings(self):
        ...
        # This relies on images being named in a consistent pattern.
        # Adjust as needed if your filenames differ.
        # For demonstration, these images are assumed to be set up similarly to other checkbox checks.

        all_good = True
        for checkbox_name, should_be_checked in desired_checkboxes.items():
            self.logger.debug(f"⚙️ Ensuring '{checkbox_name}' is {'checked' if should_be_checked else 'unchecked'}.")

            # The code assumes you have images for these checkboxes set similarly to PO_LOG_CHECKBOX, etc.
            # If not, you'd need to define them. This is a placeholder logic.
            # Example image naming: invoice_check.png, invoice_uncheck.png, etc.
        ...
        if all_good:
            self.logger.info("✅ All desired settings are in place.")
        else:
            self.logger.warning("⚠️ Some settings could not be corrected.")
    ...
    def ensure_no_subtotal(self):
        ...
        """
        try:
            # Locate the sub_total_title_image as a reference point
            ...
            if not sub_total_title_location:
                self.logger.info("ℹ️ 'Subtotal Title' not found, might already be don't subtotal.")
                return True
            ...
            pyautogui.press('d')
            self.press_enter()
            self.logger.info("📝 'Don't Subtotal' selected.")
            return True
        except Exception as e:
    ...
    def set_phase_to_actual(self, other_reports_position):
        ...
        if other_reports_position is None:
            self.logger.warning("⚠️ Cannot set phase to Actual. Other Reports position not found.")
            return
        ...
        self.press_enter()
        self.logger.info("📝 Phase set to 'Actual'.")
    ...
    def retry_set_phase_to_actual(self, other_reports_position, retries=3):
        ...
        """
        for attempt in range(1, retries + 1):
            self.logger.info(f"🔄 Attempt {attempt}/{retries} to set phase to Actual.")
            try:
                self.set_phase_to_actual(other_reports_position)

                # Check if phase is actually set to Actual by looking for the 'actual' image again.
                # If we don't find the 'actual' phase image on screen, consider it a failure.
                ...
                if actual_phase_img and pyautogui.locateOnScreen(actual_phase_img, confidence=0.95):
                    self.logger.info("✅ Phase successfully set to 'Actual'.")
                    return True
                else:
                    raise Exception("Phase not confirmed as 'Actual' after setting.")
            except Exception as e:
                self.logger.error(f"❌ Failed to set phase to Actual on attempt {attempt}: {e}")
                if attempt < retries:
                    self.logger.info("🔧 Closing and reopening the budget to retry phase setting.")
                    self.close_budget()
                    time.sleep(3)
                    # Reopen the file and try again
                    self.logger.info(f"📂 Reopening file '{self.FILE_PATH}' to retry phase setting.")
                    self.open_file(self.FILE_PATH)
                    time.sleep(self.SLEEP_TIME_OPEN)
                    # Bring up print dialog and detail reports again before retrying phase set
                    ...
                    if not other_reports_position:
                        self.logger.error("❌ Cannot find 'Detail Reports' button during retry. Aborting.")
                        return False
                else:
                    self.logger.critical("💥 All attempts to set phase to Actual have failed.")
                    return False
        return False
    ...
    def click_button_with_retry_and_reopen(self, image_path, retries=3, delay=2):
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                self.logger.warning(f"⚠️ Attempt {attempt} to click '{image_path}' failed.")
            time.sleep(delay)
    ...
    def close_budget(self):
        ...
        """
        self.logger.info("🔻 Closing Showbiz Budgeting.")
    ...
    def run(self):
        try:
            if not self.upload_only:
                self.logger.info("🧹 Ensuring a clean state before starting job.")
                self.close_budget()
                time.sleep(3)
            ...
            # Continue with the rest of the process only if phase was set successfully
            ...
            if not retry_on_failure(lambda: self.click_button(self.BUTTON_IMAGES['PREVIEW']),
                                    action_description="Clicking PREVIEW button",
                                    fallback_func=self.close_budget):
                self.report_progress("❌ Failed to click 'Preview'. Aborting.")
                return

            self.report_progress("👁️ Clicking 'Raw Text' button.")
            if not retry_on_failure(lambda: self.click_button(self.BUTTON_IMAGES['RAW_TEXT']),
                                    action_description="Clicking Raw Text button",
                                    fallback_func=self.close_budget):
            ...
            if not self.upload_only:
                self.logger.info("🧼 Cleaning up after job run. Closing Showbiz to ensure a fresh start next time.")
                self.close_budget()#endregion

#region Worker and Routes
def worker():
    """
    ...
    while True:
        ...
        # job format: (job_id, project_number, file_path, upload_only)
        if len(job) == 4:
...
if __name__ == '__main__':
    ...
    # Run Flask after starting the worker
