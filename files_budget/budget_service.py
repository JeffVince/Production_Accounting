...
#TODO get tax code aded to bill line item
...
class BudgetService(metaclass=SingletonMeta):
    ...
    def process_aggregator_detail_items(self, po_log_data: dict, session, chunk_size: int = 500):
        ...
        try:
            ...
            existing_map_updated = copy.deepcopy(existing_map_OG)
            ...
            # We will accumulate new Xero Bill Line Items separately (to be merged later)
            new_xero_bill_line_items = []
            contact_map_updated = copy.deepcopy(contact_map_OG)

            # Make sure we also create a "copy" for line items
            ...
            for d_item in detail_items_input:
                ...
                if payment_type in ["INV", "PROF", "PROJ"] and detail_state == "RTP":
                    ...
                    if key not in xero_bill_map_updated:
                        ...
                        if parent_po and parent_po.get("contact_id") and contact_xero_id != parent_po["contact_id"]:
                            differences_found = True
                        if bill_status in ["DRAFT", "SUBMITTED FOR APPROVAL", "PAID"]:
                            if differences_found:
                                ...
                                if parent_po and parent_po.get("contact_id"):
                                    existing_bill["contact_xero_id"] = parent_po["contact_id"]
                            else:
            ...
            for li in updated_xero_bill_line_items:
                bill_identifier = li.pop('_bill_identifier', None)
                if bill_identifier:
                    parent_id = bill_id_map.get(bill_identifier)
                    if parent_id is None:
                        self.logger.warning(
                            f"No parent Xero Bill found for line item with identifier {bill_identifier}"
                        )
                    else:
            ...
            try:
                for bill_id, items in xero_bill_line_items_to_create.items():
                    if not items:
                        continue
                    created_xero_bill_line_items_db = self.db_ops.bulk_create_xero_bill_line_items(
                ...
                for bill_id, items in xero_bill_line_items_to_update.items():
                    if not items:
                        continue
                    updated_xero_bill_line_items_db = self.db_ops.bulk_update_xero_bill_line_items(
                        items, session=session
                    )
                    session.flush()
            except Exception as e:
            ...
            if spend_money_to_create:
                created_spend_money_db = self.db_ops.bulk_create_spend_money(spend_money_to_create, session=session)
                session.flush()
