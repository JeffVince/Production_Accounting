...
from celery_tasks import (
    ...
    process_invoice_delete,
    process_detail_item_create,
    process_detail_item_update,
    process_detail_item_delete,
    process_purchase_order_create,
    process_purchase_order_update,
    process_purchase_order_delete,
    process_contact_create,
    process_contact_update,
    process_contact_delete,
    process_bill_line_item_create,
    process_bill_line_item_update,
    process_bill_line_item_delete,
    ...
    process_spend_money_create,
    process_spend_money_update,
    process_spend_money_delete,
...
TASK_ROUTING = {
    ...
    ("bill_line_item", "INSERT"): lambda rid: process_bill_line_item_create.delay(rid),
    ("bill_line_item", "UPDATE"): lambda rid: process_bill_line_item_update.delay(rid),
    ("bill_line_item", "DELETE"): lambda rid: process_bill_line_item_delete.delay(rid),
...
def poll_audit_log(poll_interval=5.0):
    ...
    # Determine the highest audit_log.id present, so we start from there
    ...
    while True:
        try:
            with get_db_session() as session:
                # Wrap the multiline raw SQL in text(...)
                ...
                for r in results:
                    ...
                    if key in TASK_ROUTING:
                        try:
                            ...
                            logger.exception(
                                f"Failed enqueuing Celery task for {tbl} {op}, record_id={rid}: {exc}"
                            )
                    else:
                        logger.warning(f"No route for (table={tbl}, op={op}). Skipping...")

                    # Update our last processed ID so we never handle the same row twice
...
def start_db_trigger_listener():
    ...
    poll_audit_log(poll_interval=2.0)  # Adjust interval as desired
