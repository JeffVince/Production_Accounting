...
def handle_invoice_create_or_update(invoice_id: int) -> None:
    ...
    if not invoice_record or isinstance(invoice_record, list):
        logger.warning(
            f"📄 ❌ Could not find unique Invoice with id={invoice_id}. Bailing out."
        )
        return
    ...
    if detail_matches:
        ...
        for d_item in detail_matches:
            ...
            if d_item.get("invoice_id") != invoice_id:
                ...
                logger.info(
                    f"📄 Linked invoice_id={invoice_id} to detail_item "
        ...
        logger.info(
            f"📄 💤 No matching detail items found to link for invoice_id={invoice_id}, "
    ...
    if not valid_items:
        logger.info(
            f"📄 🎉 All detail items for invoice_id={invoice_id} are RECONCILED or PAID. No sum check needed."
        )
        return
    ...
    if abs(total_sub - invoice_total) < 0.0001:
        logger.info(
            f"📄 ✅ Invoice total = sum of details for invoice_id={invoice_id}. Marking them 'RTP'."
        )
        ...
        logger.info(
            f"📄 ⚠️ Invoice total != sum of details for invoice_id={invoice_id}. Marking 'PO MISMATCH'."
        )
        for d in valid_items:
            db_ops.update_detail_item_by_keys(
                project_number,
